...
// This Class is responsible for getting a response from Safaricom and Storing the 
//Transaction Details to the Database 
class STKPush 
{ 
    public $failed = false; 
    ...
    { 
        ...
        if (property_exists($payload, 'Body') && $payload->Body->stkCallback
>ResultCode == '0') { 
            ...
            $mpesa_receipt_number = $payload->Body->stkCallback->CallbackMetadata
>Item[1]->Value; 
            $transaction_date = $payload->Body->stkCallback->CallbackMetadata
>Item[3]->Value; 
...
>Value; 
            ...
            $stkPush = MpesaSTK::where('merchant_request_id', $merchant_request_id) 
                ->where('checkout_request_id', $checkout_request_id)->first();//fetch 
//the trasaction based on the merchant and checkout ids 
